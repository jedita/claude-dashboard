...
| `generate_ai_name(prompt, file, tty)` | Runs in the background. Resolves the `claude` CLI path via `CLAUDE_BIN` (resolved at script top-level). Calls `claude -p` to generate a 3–5 word title from the user's prompt. Validates the result: rejects CLI error messages (API key, auth, network errors), titles >7 words, and conversational openers. Updates the state file and terminal tab title atomically. Guarded by an `ai_name_generated` flag to run only once per session. Logs all failures, rejections, and successes to `~/.claude/dashboard/server.log` with `[ai-name]` tag and severity level (`ERROR`, `WARN`, `INFO`). |
